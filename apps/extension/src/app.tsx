...
export function App() {
  ...
  return <main className="pinhere-popup min-h-[380px] bg-[#f4f6fa]"><header className="flex items-center justify-between border-b border-[#d8dce5] bg-[#f9fafc]/92 px-4 py-3 backdrop-blur"><Logo />{phase !== "signed_out" && phase !== "loading" && <button title="退出登录" className="focus-ring rounded-lg p-1.5 text-[#777a73] hover:bg-[#164dd80a]" onClick={() => void signOut()}><LogOut size={16} /></button>}</header><div className="p-4">
    {phase === "loading" && <div className="grid min-h-[300px] place-items-center"><LoaderCircle className="animate-spin text-[#164dd8]" /></div>}
    {phase === "signed_out" && <section className="rise pt-1"><div className="flex items-start gap-3"><div className="grid size-10 shrink-0 place-items-center rounded-xl border border-[#cbd5ec] bg-[#eaf0ff]"><ShieldCheck size={19} className="text-[#164dd8]" /></div><div><div className="font-mono text-[9px] uppercase tracking-[.15em] text-[#164dd8]">PINHERE ACCESS</div><h1 className="mt-1 text-[22px] font-extrabold leading-none tracking-[-.045em]">连接扩展，开始圈选</h1><p className="mt-2 text-xs leading-5 text-[#697080]">仅授予读取项目、创建缺陷和上传截图的权限。</p></div></div><div className="mt-4 rounded-lg border border-[#d8dce5] bg-white/70 px-3 py-2 font-mono text-[9px] leading-4 text-[#687184]">授权在 Pinhere 中完成 · 不读取网页账号或 Console</div><Button className="mt-4 w-full" onClick={() => void authorize()}>授权并登录 <ExternalLink size={15} /></Button></section>}
    {phase === "ready" && <section className="rise pt-0"><div className="mb-4 rounded-[14px] border border-[#24314d] bg-[#172033] p-4 text-white shadow-[0_10px_26px_rgba(20,34,64,.18)]"><div className="mb-5 flex items-center justify-between"><span className="font-mono text-[9px] uppercase tracking-[.14em] text-white/45">DOM CAPTURE / READY</span><span className="size-1.5 animate-pulse rounded-full bg-[#80a0ff]" /></div><div className="flex items-start gap-3"><MousePointer2 size={22} className="mt-0.5 shrink-0 text-[#9bb3ff]" /><div><h1 className="text-[21px] font-extrabold leading-tight tracking-[-.04em]">选择页面上的问题</h1><p className="mt-1.5 text-xs leading-5 text-white/58">圈选后面板自动收起；再次点击图标继续填写。</p></div></div></div><Button className="w-full" onClick={() => void startCapture()}><Crosshair size={17} />圈选页面问题</Button><div className="mt-3 text-center font-mono text-[9px] leading-4 text-[#858c9b]">不支持 Chrome 内部页、iframe 与关闭的 Shadow DOM</div></section>}
