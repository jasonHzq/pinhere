...
export function App() {
  ...
  return <main className="pinhere-popup bg-[#fafafa]"><header className="flex items-center justify-between border-b border-[#e7e7e7] bg-white px-3 py-2.5"><Logo />{phase !== "signed_out" && phase !== "loading" && <button title="退出登录" className="focus-ring rounded-md p-1.5 text-[#727272] hover:bg-black/5" onClick={() => void signOut()}><LogOut size={16} /></button>}</header><div className="p-3">
    ...
    {phase === "signed_out" && <section className="signed-out rise"><div className="flex items-start gap-2.5"><div className="grid size-9 shrink-0 place-items-center rounded-[10px] bg-[#151515] text-white"><ShieldCheck size={17} /></div><div><div className="font-mono text-[8px] font-bold uppercase tracking-[.16em] text-[#777]">Extension access</div><h1 className="mt-1 text-[19px] font-extrabold leading-none tracking-[-.045em]">连接，然后圈选</h1><p className="mt-1.5 text-[11px] leading-4 text-[#6a6a6a]">读取项目，创建缺陷并上传截图。</p></div></div><div className="mt-3 border-y border-[#e6e6e6] py-2 font-mono text-[9px] text-[#777]">授权在 Pinhere 完成 · 仅首次需要</div><Button className="mt-3 w-full" onClick={() => void authorize()}>授权并登录 <ExternalLink size={14} /></Button></section>}
