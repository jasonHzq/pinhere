...
import { AlertTriangle, Check, Clipboard, Crosshair, ExternalLink, Home, LoaderCircle, LogOut, MousePointer2, RotateCcw } from "lucide-react";
...
export function App() {
  ...
  return <main className="pinhere-popup bg-[#fafafa]"><header className="flex items-center justify-between border-b border-[#e7e7e7] bg-white px-3 py-2.5"><Logo />{phase !== "signed_out" && phase !== "loading" && <div className="flex items-center gap-0.5"><button type="button" title="打开 Pinhere 工作台" aria-label="打开 Pinhere 工作台" className="focus-ring rounded-md p-1.5 text-[#727272] transition-colors hover:bg-black/5 hover:text-[#151515]" onClick={() => void chrome.tabs.create({ url: `${BASE_URL}/zh-CN/app` })}><Home size={16} /></button><button type="button" title="退出登录" aria-label="退出登录" className="focus-ring rounded-md p-1.5 text-[#727272] transition-colors hover:bg-black/5 hover:text-[#151515]" onClick={() => void signOut()}><LogOut size={16} /></button></div>}</header><div className="p-3">
