      ...
      {initial.projects.length === 0 ? <div className="border border-dashed border-[#aaa9a2] bg-white/70 p-12 text-center"><h2 className="text-lg font-bold">{en ? "Create a project before capturing an issue" : "先创建项目，再从网页圈选问题"}</h2><a className="mt-4 inline-block text-sm font-bold underline decoration-1 underline-offset-4" href={`/${locale}/app/projects`}>{en ? "Open projects" : "前往项目设置"}</a></div> : (
        <div className="grid gap-4 lg:grid-cols-3">
          {columns.map((column) => {
            ...
            return <section key={column.status} className="min-h-[420px] rounded-xl border border-[#deded9] bg-[#fafafa] p-3"><div className="mb-3 flex items-start justify-between border-b border-[#e4e4e0] px-1 pb-3 pt-1"><div><div className="flex items-center gap-2"><h2 className="text-sm font-bold uppercase tracking-[.025em]">{en ? column.en : column.zh}</h2><Badge variant={column.status}>{columnItems.length}</Badge></div><p className="mt-1 font-mono text-[9px] uppercase tracking-[.08em] text-[#7d7d78]">{en ? column.captionEn : column.captionZh}</p></div></div><div className="space-y-2.5">{columnItems.map((issue) => <IssueCard key={issue.id} issue={issue} />)}{!columnItems.length && <div className="grid h-24 place-items-center border border-dashed border-[#d5d5d0] bg-white font-mono text-[10px] uppercase tracking-[.12em] text-[#999993]">Empty</div>}</div></section>;
          })}
        </div>
      )}
    </main>
