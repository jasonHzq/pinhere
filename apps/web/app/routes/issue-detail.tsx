...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const events = await db.select().from(issueEvents).where(eq(issueEvents.issueId, row.issue.id)).orderBy(asc(issueEvents.createdAt));
...
function promptFor(issueId: string) {
  return `请用 Pinhere Skill 修复缺陷 ${issueId}：\n1. claimIssue 原子领取；\n2. getIssue 获取上下文；\n3. 在当前仓库修复并验证；\n4. completeIssue 写回摘要。\n无法完成时调用 releaseIssue 并说明原因。`;
}
...
export default function IssueDetail() {
  ...
  async function transition(action: "claim" | "complete" | "reopen" | "release") {
    ...
    if (response.ok) {
      ...
      await refreshEvents();
    } else setActionError(payload?.error?.message ?? (en ? "Status update failed." : "状态更新失败。"));
  }
  ...
  return <main className="min-h-screen p-5 md:p-8 xl:p-10"><Link to={`/${locale}/app`} className="font-mono text-[10px] uppercase tracking-[.14em] text-[#696d67] hover:text-[#151515]">← {en ? "Back to board" : "返回看板"}</Link>
    <header className="mt-6 flex flex-col gap-5 border-b border-[#d8d8d2] pb-7 md:flex-row md:items-start md:justify-between"><div className="max-w-3xl"><div className="mb-3 flex items-center gap-2"><Badge variant={issue.status}>{issue.status}</Badge><span className="font-mono text-[10px] text-[#777a73]">{issue.id}</span></div><h1 className="text-3xl font-bold tracking-[-.05em] md:text-5xl">{issue.title}</h1><p className="mt-4 whitespace-pre-wrap text-sm leading-7 text-[#5f635d]">{issue.description}</p></div><div className="flex shrink-0 flex-wrap gap-2"><Button variant="outline" onClick={() => void editIssue()}><Pencil size={15} />{en ? "Edit" : "编辑"}</Button><Button variant="outline" onClick={() => void copyPrompt()}>{copied ? <Check size={16} /> : <Clipboard size={16} />}{copied ? (en ? "Copied" : "已复制") : (en ? "Copy repair prompt" : "复制修复 Prompt")}</Button>{issue.status === "open" && <Button onClick={() => void transition("claim")}><ScanLine size={16} />{en ? "Claim" : "人工认领"}</Button>}{issue.status === "in_progress" && <><Button variant="outline" onClick={() => void transition("release")}><Unlock size={15} />{en ? "Release" : "释放"}</Button><Button onClick={() => void transition("complete")}><Check size={16} />{en ? "Complete" : "标记完成"}</Button></>}{issue.status === "done" && <Button onClick={() => void transition("reopen")}><RotateCcw size={16} />{en ? "Reopen" : "重新打开"}</Button>}<Button variant="danger" onClick={() => void deleteIssue()}><Trash2 size={15} /></Button></div>{actionError && <p role="alert" className="text-xs text-[#bb2d3b]">{actionError}</p>}</header>
    <div className="mt-6 grid gap-6 xl:grid-cols-[1.15fr_.85fr]"><div className="space-y-6">{initial.screenshotUrl && <Card className="overflow-hidden"><div className="border-b border-[#d8d8d2] px-5 py-3 font-mono text-[10px] uppercase tracking-[.14em] text-[#696d67]">Screenshot</div><div className="bg-[#f0f0ed] p-3"><img src={initial.screenshotUrl} className="mx-auto max-h-[680px] rounded-lg shadow-[2px_2px_0_#d0d0ca]" alt="Issue screenshot" /></div></Card>}<Card className="p-5"><h2 className="text-sm font-bold">DOM context</h2><dl className="mt-4 grid gap-4 text-xs"><div><dt className="font-mono text-[10px] text-[#8b8e87]">CSS SELECTOR</dt><dd className="mt-1 overflow-x-auto rounded-lg bg-[#f1f1ee] p-3 font-mono">{issue.dom.cssSelector}</dd></div><div><dt className="font-mono text-[10px] text-[#8b8e87]">XPATH</dt><dd className="mt-1 overflow-x-auto rounded-lg bg-[#f1f1ee] p-3 font-mono">{issue.dom.xpath}</dd></div><div><dt className="font-mono text-[10px] text-[#8b8e87]">OUTER HTML</dt><dd className="mt-1 max-h-60 overflow-auto whitespace-pre-wrap rounded-lg bg-[#171717] p-4 font-mono text-[11px] leading-5 text-[#e7e7e2]">{issue.dom.outerHTML}</dd></div></dl></Card></div>
    <aside className="space-y-5"><Card className="p-5"><h2 className="text-sm font-bold">{en ? "Source" : "来源信息"}</h2><dl className="mt-4 space-y-4 text-xs"><div><dt className="font-mono text-[10px] text-[#8b8e87]">PROJECT</dt><dd className="mt-1 font-bold">{initial.project.name}</dd></div><div><dt className="font-mono text-[10px] text-[#8b8e87]">PAGE</dt><dd className="mt-1"><a href={issue.pageUrl} target="_blank" rel="noreferrer" className="flex items-start gap-2 break-all text-[#20201e] hover:underline">{issue.pageUrl}<ExternalLink className="mt-0.5 shrink-0" size={12} /></a></dd></div><div><dt className="font-mono text-[10px] text-[#8b8e87]">CREATED</dt><dd className="mt-1">{new Date(issue.createdAt).toLocaleString()}</dd></div></dl></Card><Card className="p-5"><h2 className="text-sm font-bold">{en ? "History" : "状态历史"}</h2><ol className="mt-5 space-y-5 border-l border-[#c8c7c0] pl-5">{events.map((event) => <li key={event.id} className="relative"><span className="absolute -left-[24px] top-1 size-2 rounded-full bg-[#20201e] ring-4 ring-white" /><div className="text-xs font-bold">{event.type}</div><div className="mt-1 font-mono text-[9px] text-[#8b8e87]">{new Date(event.createdAt).toLocaleString()}</div></li>)}</ol></Card></aside></div></main>;
}
